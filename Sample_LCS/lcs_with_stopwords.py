...
print("Acronyms Detected in passage:")
...
for k in range(len(wordindex)): 
	...
	if wordindex[k] - window < 0 : 
		...
		si = False
		temp = FindLCS(acronym, leaders, k, wordindex[k] - window, wordindex[k])
	...
	if wordindex[k] + window >= numwords : 
		...
		ei = False
		temp2 = FindLCS(acronym, leaders, k, wordindex[k]+1, wordindex[k] + window +1)
